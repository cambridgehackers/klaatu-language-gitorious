...
############################################################################
    ...
    rule Type_list:
        ...
        | LPAREN type_decl (COMMA type_decl)* RPAREN {{ return type_decl }}
        )
    ...
    rule single_type_definition:
        (   Type_list [ type_decl {{ Type_list.name=type_decl }} | TOKENABLE {{ Type_list.name='enable' }} ] {{ return Type_list }}
        ...
        | NUM ( VAR {{ t = VAR }} | TYPEVAR {{ t = TYPEVAR }} )
            {{ t = Numeric(t, NUM) }}
        ) SEMICOLON {{return t}}
